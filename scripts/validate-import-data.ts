...
// Same CSV interface as import script - Updated for new CSV structure
interface CSVRow {
  market: string;
  firstName: string;
  lastName: string;
...
const activityMapping = [
  ...
  { csvField: 'casaFerrari', activityTitle: 'Casa Ferrari', type: 'yesno' },
  {
    ...
    activityTitle: 'PRS Paddock Club - Friday',
    type: 'yesno',
  },
  {
    csvField: 'regalClubFri',
    activityTitle: 'Regal Club - Friday',
    type: 'yesno',
  },
  {
    csvField: 'crystalGoldLoungeFri',
    activityTitle: 'Crystal Gold Lounge - Friday',
    type: 'yesno',
  },
  {
    csvField: 'eveningBar',
    activityTitle: 'Evening Bar - Manhattan Bar',
    type: 'venue',
    venues: [
      'Manhattan Bar',
      'Origin Bar',
  ...
  },
  {
    ...
    activityTitle: 'Fred Interview Session',
    type: 'yesno',
  },
  {
    csvField: 'prsPaddockClubSat',
    activityTitle: 'PRS Paddock Club - Saturday',
    type: 'yesno',
  },
  {
    csvField: 'regalClubSat',
    activityTitle: 'Regal Club - Saturday',
    type: 'yesno',
  },
  {
    csvField: 'crystalGoldLoungeSat',
    activityTitle: 'Crystal Gold Lounge - Saturday',
    type: 'yesno',
  },
  {
    csvField: 'sushiSambaAfterParty',
    activityTitle: 'Sushi Samba After Party',
    type: 'yesno',
  },
  {
    csvField: 'prsPaddockClubSun',
    activityTitle: 'PRS Paddock Club - Sunday',
    type: 'yesno',
  },
  {
    csvField: 'regalClubSun',
    activityTitle: 'Regal Club - Sunday',
    type: 'yesno',
  },
  {
    csvField: 'crystalGoldLoungeSun',
    activityTitle: 'Crystal Gold Lounge - Sunday',
    type: 'yesno',
  },
  {
    csvField: 'lavoAfterParty',
    activityTitle: 'LAVO After Party',
    type: 'yesno',
  },
];
 ...
 */
async function getRandomUserSample(
  eventId: string,
  ...
  console.log(
    `🎯 Sampling ${Math.min(sampleSize, totalUsers)} users for validation`
  );
 ...
 */
function findCSVRecord(
  user: any,
  csvRecords: CSVRow[]
): { record: CSVRow | null; rowNumber: number } {
  ...
  console.log(
    `🔍 Looking for user: "${firstName} ${lastName}" email: "${email}"`
  );
  ...
  for (let i = 0; i < csvRecords.length; i++) {
    ...
    if (firstName && lastName && csvFirstName && csvLastName) {
      if (firstName === csvFirstName && lastName === csvLastName) {
        ...
        console.log(
          `   ✅ Found by name: Row ${csvLineNumber} (CSV line ${csvLineNumber}) - ${csvFirstName} ${csvLastName}`
        );
  ...
  if (email && email !== '') {
    const emailMatches = csvRecords.filter(
    ...
    if (emailMatches.length === 1) {
      ...
      console.log(
        `   ✅ Found by unique email: Row ${csvLineNumber} - ${emailMatches[0].firstName} ${emailMatches[0].lastName}`
      );
      return { record: emailMatches[0], rowNumber: csvLineNumber };
    } else if (emailMatches.length > 1) {
      console.log(
        `   ⚠️ Email "${email}" matches ${emailMatches.length} CSV records - skipping email match`
      );
    }
  }
...
function validateProfile(user: any, csvRecord: CSVRow): string[] {
  ...
  if (csvFirstName !== dbFirstName) {
    issues.push(
      `First name mismatch: CSV="${csvRecord.firstName}" vs DB="${profile?.firstName}"`
    );
  }

  if (csvLastName !== dbLastName) {
    issues.push(
      `Last name mismatch: CSV="${csvRecord.lastName}" vs DB="${profile?.lastName}"`
    );
  }
  ...
  if (csvEmail !== dbEmail) {
    ...
    if (
      (csvEmail && dbEmail && csvEmail !== dbEmail) ||
      (csvEmail && !dbEmail) ||
      (!csvEmail && dbEmail)
    ) {
      issues.push(
        `Email mismatch: CSV="${csvRecord.email}" vs DB="${profile?.email}"`
      );
    }
  }
...
async function validateGroup(user: any, csvRecord: CSVRow): Promise<string[]> {
  ...
  const hasCorrectGroup = userGroups.some(
    (group) => group.name === correctedMarketName
  );

  if (!hasCorrectGroup) {
    const groupNames = userGroups.map((g) => g.name).join(', ');
    issues.push(
      `Group assignment mismatch: Expected="${correctedMarketName}" but user is in groups: [${groupNames}]`
    );
  }
...
async function validateActivities(
  ...
  if (!userExclusionsCache) {
    ...
    console.log(
      `✅ Loaded exclusions for ${userExclusionsCache.size} users (${allExclusions.length} total exclusions)`
    );
  }
  ...
  if (allActivities.length === 0) {
    issues.push(
      `SYSTEM ISSUE: No activities exist in database for event ${eventId}`
    );
    return issues;
  }
  ...
  const actualUserActivities = activitiesViaGroups.filter(
    (activity) => !userExclusions.includes(activity.id)
  );
  ...
  for (const mapping of activityMapping) {
    ...
    if (mapping.type === 'yesno') {
      ...
      const isActuallyAssigned = actualUserActivities.some(
        (activity) => activity.title === mapping.activityTitle
      );
      ...
      if (shouldBeAssigned && !isActuallyAssigned) {
        ...
        const hasGroupAccess = activitiesViaGroups.some(
          (activity) => activity.title === mapping.activityTitle
        );
        const isExcluded =
          userExclusions.length > 0 &&
          activitiesViaGroups.some(
            (activity) =>
              activity.title === mapping.activityTitle &&
              userExclusions.includes(activity.id)
          );

        if (!activityExists) {
          issues.push(
            `Missing activity: "${mapping.activityTitle}" - Activity doesn't exist in database`
          );
        } else if (!hasGroupAccess) {
          issues.push(
            `Missing activity: "${mapping.activityTitle}" - Activity exists but not assigned to user's groups`
          );
        } else if (isExcluded) {
          issues.push(
            `Missing activity: "${mapping.activityTitle}" - User is excluded from this activity`
          );
        } else {
          issues.push(
            `Missing activity: "${mapping.activityTitle}" - Unknown reason`
          );
        }
      } else if (!shouldBeAssigned && isActuallyAssigned) {
        issues.push(
          `Unexpected activity: "${mapping.activityTitle}" - CSV has "N" but user can see activity`
        );
      }
      ...
      if (expectedVenue && mapping.venues.includes(expectedVenue)) {
        ...
        const isActuallyAssigned = actualUserActivities.some(
          (activity) => activity.title === expectedActivityTitle
        );
        ...
        if (!isActuallyAssigned) {
          ...
          const isExcluded =
            ...
            activitiesViaGroups.some(
              (activity) =>
                activity.title === expectedActivityTitle &&
                userExclusions.includes(activity.id)
            );

          if (!activityExists) {
            issues.push(
              `Missing evening bar: "${expectedActivityTitle}" - Activity doesn't exist in database`
            );
          } else if (!hasGroupAccess) {
            issues.push(
              `Missing evening bar: "${expectedActivityTitle}" - Activity exists but not assigned to user's groups`
            );
          } else if (isExcluded) {
            issues.push(
              `Missing evening bar: "${expectedActivityTitle}" - User is excluded from this activity`
            );
          } else {
            issues.push(
              `Missing evening bar: "${expectedActivityTitle}" - Unknown reason`
            );
          }
        }
      }
...
function validateCar(user: any, csvRecord: CSVRow): string[] {
  ...
  console.log(
    `   CSV car: "${csvRecord.marketCarNumber}" → cleaned: "${expectedCar}"`
  );
  ...
  if (!expectedCar || expectedCar === 'N/A') {
    ...
    if (user.carNumbers && user.carNumbers.length > 0) {
      ...
      issues.push(
        `Unexpected car: CSV has no car but user assigned to cars: [${user.carNumbers.join(', ')}]`
      );
    } else {
    ...
    if (!user.carNumbers || user.carNumbers.length === 0) {
      ...
      issues.push(
        `Missing car: CSV specifies car "${expectedCar}" but user has no cars assigned`
      );
    } else if (!user.carNumbers.includes(expectedCar)) {
      console.log(`   ❌ Issue: Wrong car assigned`);
      issues.push(
        `Wrong car: CSV specifies "${expectedCar}" but user assigned to: [${user.carNumbers.join(', ')}]`
      );
    } else {
...
function validateHotel(user: any, csvRecord: CSVRow): string[] {
  ...
  console.log(
    `   CSV room type: "${csvRecord.roomCategory}" → cleaned: "${expectedRoomType}"`
  );
  console.log(
    `   CSV accommodation required: "${csvRecord.accommodationRequired}"`
  );
  ...
  if (!user.roomAssignments || user.roomAssignments.length === 0) {
    ...
    issues.push(
      `Missing room assignment: CSV specifies room type "${expectedRoomType}" but no room assigned`
    );
  } else {
    ...
    if (actualRoomType !== expectedRoomType) {
      ...
      issues.push(
        `Room type mismatch: CSV="${expectedRoomType}" vs DB="${actualRoomType}"`
      );
    } else {
...
function generateReport(results: ValidationResult[]): void {
  ...
  console.log(
    `   Perfect Users (no issues): ${perfectUsers} (${((perfectUsers / totalUsers) * 100).toFixed(1)}%)`
  );
  console.log(
    `   Users with Issues: ${totalUsers - perfectUsers} (${(((totalUsers - perfectUsers) / totalUsers) * 100).toFixed(1)}%)`
  );
  ...
  Object.entries(issueStats).forEach(([category, count]) => {
    ...
    console.log(
      `   ${category.toUpperCase()} Issues: ${count}/${totalUsers} (${percentage}%)`
    );
  });
...
async function debugActivitySystem(eventId: string): Promise<void> {
  ...
  if (activities.length === 0) {
    ...
    console.log(
      `   💡 Activities failed to create during import due to ObjectId errors`
    );
    return;
  }
  ...
  const activitiesWithoutGroups = activities.filter(
    (a) => a.groupIds.length === 0
  );

  console.log(
    `   📊 Activities with group assignments: ${activitiesWithGroups.length}`
  );
  console.log(
    `   📊 Activities without group assignments: ${activitiesWithoutGroups.length}`
  );
  ...
  activities.slice(0, 10).forEach((activity) => {
    console.log(
      `      - "${activity.title}" (groups: [${activity.groupIds.join(', ')}]) created: ${activity.createdAt}`
    );
  });
}
...
async function main() {
  try {
    ...
    if (!event) {
      throw new Error(
        'Event not found. Make sure import completed successfully.'
      );
    }
    ...
    const [totalUsers, totalGroups, totalActivities, totalRoomAssignments] =
      await Promise.all([
    ...
    if (totalActivities === 0) {
      console.log(
        `   ❌ CRITICAL: No activities found! Import likely failed during activity creation.`
      );
      console.log(`   🔧 This explains why all users show missing activities.`);
      console.log(
        `   💡 Check import logs for activity creation errors (ObjectId issues, etc.)`
      );
    }
    ...
    if (totalActivities > 0) {
      ...
      sampleActivities.forEach((activity) => {
        console.log(
          `      - "${activity.title}" (groups: [${activity.groupIds.join(', ')}])`
        );
      });
    }
    ...
    for (const user of users) {
      ...
      if (!record) {
        console.log(
          `⚠️ Could not find CSV record for user: ${(user.profile as any)?.firstName} ${(user.profile as any)?.lastName}`
        );
        continue;
      }

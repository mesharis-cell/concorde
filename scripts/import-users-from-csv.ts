...
function mapCSVRowToUser(row: CSVRow, rowNumber: number): CreateUser | null {
    try {
        ...
        const inboundDepartureDate = validateDateFormat(row['Inbound Departure date [dd/mm/yyy]']);
        const inboundDepartureTime = validateTimeFormat(row['Inbound Departure time [24hr hh:mm]']);
        const inboundArrivalDate = validateDateFormat(row['Inbound Arrival date [dd/mm/yyy]']);
        const inboundArrivalTime = validateTimeFormat(row['Inbound Arrival time [24hr hh:mm]']);
        const outboundDepartureDate = validateDateFormat(row['Outbound Departure date [dd/mm/yyy]']);
        const outboundDepartureTime = validateTimeFormat(row['Outbound Departure time [24hr hh:mm]']);
        const outboundArrivalDate = validateDateFormat(row['Outbound Arrival date [dd/mm/yyy]']);
        const outboundArrivalTime = validateTimeFormat(row['Outbound Arrival time [24hr hh:mm]']);
            ...
            .on('end', async () => {
                ...
                for (let i = 0; i < rows.length; i++) {
                    ...
                    try {
                        ...
                        console.log(`ðŸ¨ Row ${rowNumber} (${userEmail}): Room assignment analysis:`);
                        ...
                        if (accommodationRequired && roomCategory && validRoomTypes.includes(roomCategory)) {
                            ...
                            try {
                                console.log(`   - âœ… ATTEMPTING room assignment: ${roomCategory}`);
                                ...
                                console.log(`   - âœ… SUCCESS: Room assigned! Assignment ID: ${roomAssignment.id}`);
                                console.log(`ðŸ¨ Row ${rowNumber}: âœ… SUCCESSFULLY assigned ${roomCategory} room to ${userEmail}`);
                            } catch (roomError: any) {
                                stats.roomAssignments.failed++;
                                console.log(`   - âŒ FAILED: ${roomError.message}`);
                                console.warn(`âš ï¸  Row ${rowNumber}: Room assignment FAILED - ${roomError.message}`);
                            ...
                            console.log(`ðŸ¨ Row ${rowNumber}: â­ï¸  NO room assignment for ${userEmail} - ${reason}`);
                        }

                        stats.successful++;
                        console.log(`âœ… Row ${rowNumber}: Created user ${userData.profile?.email || userData.profile?.firstName || 'Unknown'}`);
                        ...
                        console.error(`âŒ Row ${rowNumber}: Failed to create user - ${error.message}`);
                    }
                }
...
async function main() {
    ...
    try {
        ...
        console.log(`   â­ï¸  Skipped (No Accommodation): ${stats.roomAssignments.skippedNoAccommodation}`);
        console.log(`   â­ï¸  Skipped (Invalid Room Type): ${stats.roomAssignments.skippedInvalidRoomType}`);
        ...
        if (stats.roomAssignments.assignments.length > 0) {
            ...
            stats.roomAssignments.assignments.forEach(assignment =>
                console.log(`   Row ${assignment.row}: ${assignment.user} â†’ ${assignment.roomType} (ID: ${assignment.assignmentId})`)
            );
        }
...
export { importUsersFromCSV, mapCSVRowToUser, parseEmergencyContact, parseDate, validateDateFormat, validateTimeFormat };